...
class AppIntegrationTest extends Specification {
    ...
    def "create empty account is successful"() {
        ...
        Response adamAccount = createAccount("{\"currency\":\"PLN\"}")
        then: "account is created with no found"
        adamAccount.code() == 201
        adamAccount.body().string().contains("\"balance\":{\"currency\":\"PLN\",\"amount\":0}")

        when: "beth creates new account"
        Response bethAccount = createAccount("{\"currency\":\"PLN\", \"initialBalance\":345.67}")
        ...
        bethAccountAfterTransfer.body().string().contains("\"balance\":{\"currency\":\"PLN\",\"amount\":332.22}")

        when: "adam checks his account"
        Response adamAccountAfterTransfer = checkAccount(1)
        then: "transfer is successful"
        adamAccountAfterTransfer.code() == 200
        adamAccountAfterTransfer.body().string().contains("\"balance\":{\"currency\":\"PLN\",\"amount\":13.45}")
